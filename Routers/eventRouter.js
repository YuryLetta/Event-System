...
eventRouter.get("/edit/:_id",(request,response)=>{
    eventModel.find({_id:request.params._id}).populate({path : "mainSpeaker otherSpeakers"}).then((events)=>{
        speakerModel.find({}).then((speakers)=>{
            response.render("events/editEvent",{eventData:events[0],moment:moment,speakers:speakers});
        }).catch((err)=>{
            response.send(" "+err);
         });
    }).catch((err)=>{
...
eventRouter.get("/delete/:_id",(request,response)=>{
   eventModel.deleteOne({_id: request.params._id}).then((data)=>{
    response.send("Done");
  }).catch((err)=>{
     response.send(" "+err);
  });
...
eventRouter.get("/list",(request,response)=>{
    eventModel.find({}).populate({path:"mainSpeaker otherSpeakers"}).then((data)=>{
        response.render("events/listEvents",{eventData:data})
    }).catch((err)=>{
       response.send(" "+err);
    });
})
